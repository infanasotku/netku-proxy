...
sops -d deploy/secrets/prod/common.secrets.yaml > "${TMP_SECRETS_PATH}"
...
#  -f deploy/values/prod/common.values.yaml \
#  -f deploy/values/prod/api.values.yaml \
#  --namespace netku-prod
...
# helmfile -f helmfile.yaml.gotmpl --state-values-set tag=test-tag -e prod --skip-refresh -l svc=api template
